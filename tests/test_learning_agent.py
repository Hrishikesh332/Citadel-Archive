...
def agent(
    *,
...
) -> tuple[LearningAgent, FakeCitadel, FakeSyncer, FakeGateway]:
    ...
    return learning_agent, fake_citadel, fake_syncer, fake_gateway


async def test_run_happy_path_builds_digest_and_posts_to_gateway() -> None:
    learning_agent, fake_citadel, fake_syncer, fake_gateway = agent()
    ...
    # Vault search results feed the digest packet by title, never raw body.
...
async def test_run_preview_mode_does_not_post() -> None:
    learning_agent, _, _, fake_gateway = agent()
...
async def test_github_sync_failure_propagates() -> None:
    learning_agent, _, _, _ = agent(syncer=FakeSyncer(error=RuntimeError("github down")))
...
async def test_vault_search_failure_degrades_to_empty_context() -> None:
    learning_agent, _, _, _ = agent(citadel=FakeCitadel(search_error=RuntimeError("cognee down")))
...
async def test_gateway_exception_is_reported_not_raised() -> None:
    learning_agent, _, _, _ = agent(gateway=FakeGateway(error=RuntimeError("chat down")))
